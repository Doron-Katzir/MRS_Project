...
% hiding any model assumptions. The residual after projection onto b provides
% an initial estimate of spectral noise, and the spread of projected
% concentrations provides an initial estimate of population variance.
bTb = b.' * b;
cOLS = (b.' * S / bTb).';                % Also used for comparison below
muInitial = mean(cOLS);
projectionResidual = S - b * cOLS.';
sigma2Initial = mean(projectionResidual(:).^2);
tau2Initial = max(var(cOLS, 1) - sigma2Initial / bTb, 1e-6);
sigma2Initial = max(sigma2Initial, 1e-6);

thetaInitial = [muInitial, log(tau2Initial), log(sigma2Initial)];
objective = @(theta) MarginalNegativeLogLikelihood(theta, S, b);
...
[thetaHat, nllHat, exitFlag, optimizerOutput] = fminsearch( ...
    objective, thetaInitial, optimizerOptions);
...
function negativeLogLikelihood = MarginalNegativeLogLikelihood(theta, S, b)
%MARGINALNEGATIVELOGLIKELIHOOD Full IID marginal Gaussian objective.
%   S is K x T and b is K x 1. No explicit matrix inverse is formed.
    ...
    V = tau2 * (b * b.') + sigma2 * eye(K);
    [L, cholFlag] = chol(V, "lower");
    ...
    residuals = S - mu * b;       % K x T; MATLAB implicit expansion
    whitenedResiduals = L \ residuals;

    logDetV = 2 * sum(log(diag(L)));
    quadraticTerm = sum(whitenedResiduals(:).^2);
