...
cacheOptions = ParseCoordCacheOptions(cfg, varargin{:});
...
for pIdx = 1:height(patients)
    ...
    coordStageTimer = tic;
    [quant, fitData, coordFiles, badCoordTable, cacheEvent] = ReadCoordWithPatientCache( ...
        coordDir, ...
        patientID, ...
        cacheOptions, ...
...
function [quant, fitData, coordFiles, badCoordTable, cacheEvent] = ...
    ReadCoordWithPatientCache(coordDir, patientID, cacheOptions, varargin)
    ...
    parseTimer = tic;
    [quant, fitData, coordFiles, badCoordTable] = readCoord( ...
        coordDir, ...
        'filePrefix', filePrefix, ...
        'coordFiles', sourceCoordFiles);
    cacheEvent.parseSeconds = toc(parseTimer);

    if cacheOptions.enabled
        writeTimer = tic;
        cacheEvent.didAttemptWrite = true;
        cacheRecord = BuildPatientCoordCacheRecord( ...
            patientID, ...
            coordDir, ...
            filePrefix, ...
            sourceMetadata, ...
        ...
        cacheEvent.writeSucceeded = WritePatientCoordCacheAtomic(cacheFile, cacheRecord);
        cacheEvent.writeSeconds = toc(writeTimer);
    end
end


function [quant, fitData, coordFiles, badCoordTable] = readCoord(coordDir, varargin)

    p = inputParser;
    p.addParameter('filePrefix', "", @(x) ischar(x) || isstring(x));
    p.addParameter('coordFiles', strings(0, 1), ...
        @(x) ischar(x) || isstring(x) || iscellstr(x));
    parse(p, varargin{:});

    filePrefix = string(p.Results.filePrefix);
    coordFiles = string(p.Results.coordFiles(:));

    if isempty(coordFiles)
        coordFiles = DiscoverCoordFiles(coordDir, filePrefix);
    end

    % First try reading all files together.
    ...
    % If batch reading failed, identify the bad file/files.
...
function [coordFiles, coordInfo] = DiscoverCoordFiles(coordDir, filePrefix)
    ...
    % Keep only the expected LCModel division files.
...
function [isValid, reason, isCorrupt] = ValidatePatientCoordCache( ...
    ...
    if ~istable(cached.sourceMetadata) || ...
            ~isequaln(cached.sourceMetadata, sourceMetadata)
        reason = "source metadata changed";
        return;
    end

    if ~isstruct(cached.parserMetadata) || ...
            ~isequaln(cached.parserMetadata, parserMetadata)
        reason = "VDIIO parser metadata changed";
        return;
    end

    if ~isa(cached.quant, 'VDIQuant') || ...
            ~isprop(cached.quant, 'metabTable') || ...
...
function cacheRecord = BuildPatientCoordCacheRecord( ...
    ...
    cacheRecord.createdAtUTC = char(datetime('now', ...
        'TimeZone', 'UTC', ...
...
function wasWritten = WritePatientCoordCacheAtomic(cacheFile, coordCache)
    ...
    try
        ...
        cleanupObj = onCleanup(@() DeleteCoordCacheTempFile(tempCacheFile)); %#ok<NASGU>
...
function cacheOptions = ParseCoordCacheOptions(cfg, varargin)
