...
baselineDiagCfg.runAdjustedPairwise = true;
...
%% Baseline-PC-residual empirical-vs-LCModel/de Graaf correlation test
% This is a parallel diagnostic/export. The original pairwise result above
% and its CSV files are not modified.

if baselineDiagCfg.runAdjustedPairwise
    if ~baselineDiagCfg.enabled || ~baselineDiagCfg.doMetaboliteRegression || ...
            ~baselineDiagCfg.doPairResidualization
        error(['runAdjustedPairwise requires the baseline diagnostic, ', ...
            'metabolite regression, and pair residualization to be enabled.']);
    end

    adjustedPairCfg = statsCfg.pairwise;
    adjustedPairCfg.empiricalSignalSource = "baselinePCResidual";
    adjustedPairCfg.empiricalCorrelationTable = ...
        baselineDiagnostics.pairResidualizationTable;
    adjustedPairCfg.outputDir = fullfile( ...
        pwd, "PairwiseEmpiricalVsModelResults_BaselineAdjusted");

    baselineAdjustedPairOutputs = ...
        TestPairwiseEmpiricalVsModelCorrelation( ...
        analysisData, adjustedPairCfg);

    originalSummary = pairwise.summaryTable;
    adjustedSummary = baselineAdjustedPairOutputs.pairSummaryTable;
    separator = string(char(31));
    originalKeys = string(originalSummary.metaboliteA) + separator + ...
        string(originalSummary.metaboliteB);
    adjustedKeys = string(adjustedSummary.metaboliteA) + separator + ...
        string(adjustedSummary.metaboliteB);
    [isCommonPair, adjustedLocations] = ismember( ...
        originalKeys, adjustedKeys);
    originalLocations = find(isCommonPair);
    adjustedLocations = adjustedLocations(isCommonPair);

    baselineAdjustedPairComparison = struct();
    baselineAdjustedPairComparison.nOriginalSummaryRows = ...
        height(originalSummary);
    baselineAdjustedPairComparison.nAdjustedSummaryRows = ...
        height(adjustedSummary);
    baselineAdjustedPairComparison.nCommonPairs = sum(isCommonPair);
    baselineAdjustedPairComparison.rowsAlignInCurrentOrder = ...
        isequal(originalKeys, adjustedKeys);
    baselineAdjustedPairComparison.matchedPairLabelsAlign = all( ...
        originalSummary.metaboliteA(originalLocations) == ...
        adjustedSummary.metaboliteA(adjustedLocations) & ...
        originalSummary.metaboliteB(originalLocations) == ...
        adjustedSummary.metaboliteB(adjustedLocations));

    modelDifferences = [ ...
        abs(originalSummary.groupModelR(originalLocations) - ...
        adjustedSummary.groupModelR(adjustedLocations)); ...
        abs(originalSummary.meanModelR(originalLocations) - ...
        adjustedSummary.meanModelR(adjustedLocations))];
    baselineAdjustedPairComparison.maxAbsModelCorrelationDifference = ...
        max(modelDifferences, [], 'omitnan');
    baselineAdjustedPairComparison.meanDeltaZOriginal = mean( ...
        originalSummary.meanDeltaZ_empMinusModel, 'omitnan');
    baselineAdjustedPairComparison.meanDeltaZBaselineAdjusted = mean( ...
        adjustedSummary.meanDeltaZ_empMinusModel, 'omitnan');

    fprintf('\nOriginal-versus-baseline-adjusted pairwise consistency\n');
    fprintf('Original summary rows: %d\n', ...
        baselineAdjustedPairComparison.nOriginalSummaryRows);
    fprintf('Adjusted summary rows: %d\n', ...
        baselineAdjustedPairComparison.nAdjustedSummaryRows);
    fprintf('Common metabolite pairs: %d\n', ...
        baselineAdjustedPairComparison.nCommonPairs);
    fprintf('Rows align in current table order: %d\n', ...
        baselineAdjustedPairComparison.rowsAlignInCurrentOrder);
    fprintf('Matched metabolite labels align: %d\n', ...
        baselineAdjustedPairComparison.matchedPairLabelsAlign);
    fprintf('Maximum absolute model-correlation difference: %.12g\n', ...
        baselineAdjustedPairComparison.maxAbsModelCorrelationDifference);
    fprintf('Original mean meanDeltaZ_empMinusModel: %.5f\n', ...
        baselineAdjustedPairComparison.meanDeltaZOriginal);
    fprintf('Adjusted mean meanDeltaZ_empMinusModel: %.5f\n', ...
        baselineAdjustedPairComparison.meanDeltaZBaselineAdjusted);
end
