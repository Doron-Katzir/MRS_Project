...
%
% Here:
%   X_ij      = one LCModel concentration value
%   i         = division size
%   j         = part inside that division
%   beta0     = reference division mean, usually Division_36
%   alpha_i   = fixed bias of division i relative to reference
%   sigma_i^2 = variance inside division i
...
heteroResultsSig = struct;
skippedHeteroMetabs = strings(0, 1);
skippedHeteroReasons = strings(0, 1);
...
for i = 1:numel(analysisMetabList)
    ...
    try
        heteroResultsSig.(fieldName) = FitDivisionHeteroscedasticFixedEffect( ...
            modelTableSig, ...
            metabName, ...
            'referenceDivision', referenceDivision);

    catch ME
        if contains(string(ME.message), "Reference division") || ...
                contains(string(ME.message), "was not found in modelTable")

            warning('Skipping metabolite "%s": %s', metabName, ME.message);
            skippedHeteroMetabs(end+1, 1) = metabName; %#ok<SAGROW>
            skippedHeteroReasons(end+1, 1) = string(ME.message); %#ok<SAGROW>
...
skippedHeteroTable = table(skippedHeteroMetabs, skippedHeteroReasons, ...
    'VariableNames', {'metabolite', 'reason'});

if ~isempty(skippedHeteroTable)
    disp('Skipped metabolites in heteroscedastic fixed-effect model:')
    disp(skippedHeteroTable)
end
